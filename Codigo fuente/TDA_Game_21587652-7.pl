...
verificación1(Game,Player,Colunma,[J1,J2,NewBoard,Turno,NewHistorial]):-
    get_current_player(Game,VerificacionPlayer),
    VerificacionPlayer = Player,
    get_P1(Game,Player1),
    get_P2(Game,J2),
    Player1 = Player,
    get_nombre(Player,Nombre),
    restar_pieza(Player1,J1),
    game_get_board(Game,Board),
    get_color(Player,Pieza),
    play_piece(Board,Colunma,Pieza,NewBoard),
    game_history(Game,Historial),
    Jugada = [Nombre,Colunma],
